(function ($) {
	...
	var portfolio_grid = function ($scope, $) {
		...
		if (portfolio_grid_options && portfolio_grid_options.show_filter_bar === "yes") {
			...
			if ($grid.length && $grid.find("img").length) {
				var bindPortfolioFilter = function () {
					if ($.fn.isotope) {
						$grid.isotope({
					...
					$portfolio_filter.off("click.awaikenPortfolio").on("click.awaikenPortfolio", "li", function () {
						...
						$grid.find(".awaiken-portfolio-grid-item").each(function () {
							var $item = $(this);
							var showItem = filterValue === "*" || $item.is(filterValue);
							...
							if (showItem) {
								$item
									...
									.animate({ opacity: 1 }, {
										duration: 260,
										step: function (now) {
											var offset = 16 - now * 16;
											var scale = 0.98 + now * 0.02;
											$item.css("transform", "translateY(" + offset + "px) scale(" + scale + ")");
										},
										complete: function () {
											$item.css("transform", "");
										},
				...
				if ($.fn.imagesLoaded) {
					$grid.imagesLoaded(bindPortfolioFilter);
				} else {
					bindPortfolioFilter();
				}
			}
	...
	$window.on("elementor/frontend/init", function () {
		elementorFrontend.hooks.addAction(
			"frontend/element_ready/imigo-countries-grid.default",
			portfolio_grid
		);
	});
